...
//  private TextView mFileName;
  ...
  private int VIDEO_LAYOUT_TYPE = 0;
  ...
  public MediaController(Context context, boolean fromXml, View container, VideoView videoView, Activity activity) {
    super(context);
    initController(context);
...
//      mFileName.setText(mTitle);
    mFullScreen = (ImageView) v.findViewById(getResources().getIdentifier("mediacontroller_all_screen","id",mContext.getPackageName()));
    if (mFullScreen != null){
      mFullScreen.setOnClickListener(new OnClickListener() {
        @Override
        public void onClick(View v) {
          if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            mActicity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE);
            mVideoView.setVideoLayout(VideoView.VIDEO_LAYOUT_STRETCH, 0);
          }
          if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE){
            mActicity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
            mVideoView.setVideoLayout(VideoView.VIDEO_LAYOUT_STRETCH,0);
          }
        }
      });
